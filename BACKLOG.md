...
| B-11 | UTC date drift on delayed cron runs | med | ⏳ | A scheduled run starting after ~23:00 UTC writes output under the next day's filename (08-26 → 08-27 file, 08-31 → 09-01 file); the next day's run then overwrites it, losing a session's data. Derive the filename from the market session, not `date.today()` at write time. |
...
| ID | Item | Commit | Notes |
|---|---|---|---|
| D-19 | B-04: SPY/QQQ/IWM/TNA index tile row on dashboard | `ca0fc14` | `_fetch_index_tiles()` scrapes Finviz live at generation time. Shows price, day %, week %, vs 50MA. Non-fatal fallback. 4-tile compact row inside Market State section. |
