...
- `utils/generators/generate_live_portfolio.py` — Live SnapTrade dashboard. `write_page()` pulls SnapTrade account balances + Finviz live quote/ATR%/SMA20%/Stage; renders `data/live_portfolio.html` (light theme). Verdict logic mirrors `/pos-review` skill (🚨 CUT / 💰 PEEL ½ / 🟢 trail tighter / ⚠ peel ⅓ / ✅ working / 🟡 sleeping / 💀 dead weight). Non-fatal placeholder on fetch failure. Called from `position_monitor.py` on every run (3× daily book + 30-min critical). Linked from `index.html` as **Live Portfolio**. **2026-06-19:** added **Entry/Held** columns (ticker→entry_date from `positions.json` open_positions; `—` for SnapTrade-only names like leveraged ETFs not in the rules book), a **column legend** (S20% = price vs 20-day MA, %Bk = % of book, St = Weinstein stage, etc.), and rows now sort **decision-first**. `classify_action(gain, atr)` is the single source for both the summary chips AND per-row `data-action` tags — it mirrors `verdict_for`'s EXACT clause order (`gain≥10 → trail` checked before the high-vol peel clause, so a +11% high-ATR name is `trail`, NOT `peel`), so the "PEEL/CUT/dead" chip counts can never disagree with the Verdict column. Chips are clickable to filter the table by action.
- `utils/generators/generate_portfolio.py` — Claude paper (Alpaca) dashboard → `data/claude_portfolio.html`. **2026-06-19:** Open Positions gained **Entry Date/Held** columns (`open_entry_dates()` replays FILL activities FIFO and reports the oldest still-open lot date — Alpaca `/positions` carries no entry date and `paper_stops.json` dates are often null). Trade History rows now carry `data-month`/`data-sort`; the **month chart bars + month-table rows are clickable to filter trades to that month** (`filterMonth`), and column headers are **click-to-sort** (`sortTable`, date/num/str). Tests: `tests/test_generate_portfolio.py`.
