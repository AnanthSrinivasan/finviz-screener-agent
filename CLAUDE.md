...
- **21 EMA trail mode (2026-07-10 — spec [docs/specs/ema21-trail-mode.md](docs/specs/ema21-trail-mode.md), VIK 7/2 exit — PAPER LAB ONLY):** low-vol confirmed runners (`atr_pct ≤ 5` AND `peak ≥ +20`) graduate from the ATR tier trail to a close-based 21 EMA trail — sticky `trail_mode="ema21"` in `paper_stops.json`. `rules.ema21_trail_activates()` / `rules.ema21_trail_verdict(closes, lows, close, atr, peak, breach_count)` (post-close pass only): exit on close < 21 EMA **+ lower low** vs 10-session swing low (EMA break holding prior lows = shakeout → HOLD), or 3 consecutive closes camping below the EMA (counter resets on reclaim). `rules.ema21_mode_floor()` = BE/loss-cap/peak×0.90 floors — checked every run incl. intraday, ATR trail excluded by design; `stop_price` keeps ratcheting as the shadow counterfactual logged per post-close run to `data/trail_mode_ab.json`. Review gate: 8 weeks or 5 exits, then discuss manual/live. Layer 1b alert skipped in-mode. Slack `📐 [PAPER] 21EMA TRAIL EXIT`. Tests: `tests/test_ema21_trail.py` (16).
...
**Supporting files:**
- `utils/generators/generate_index.py` — Slim landing page (cx-rehaul 2026-07-15): ONE hero button (☀️ Open Cockpit) + shared nav + latest-weekly card + 📚 Report Archive (`<details>` groups keeping EVERY dated report reachable). Also writes `data/record.html` via `generate_record.py` (tabs: Performance / Perf 2024-25 / MAE-MFE / Trader Mirror, lazy iframes). The Performance page itself (`utils/generate_performance.py`) has its own period tabs — YTD (default) / Lifetime / each prior year — added 2026-08-18 after it was found rendering lifetime totals under a "2026 YTD" title (+$69,857 shown vs −$20,579 actual YTD).
- `utils/generators/nav.py` + `utils/generators/theme.py` — shared nav bar (`render_nav(active, at_root)`) and one dark design system (`BASE_CSS` marker `BASE_CSS v1` + `page_shell`), adopted by index/cockpit/etf_rotation/watchlist/portfolios/chart grid/weekly/record. `portfolio_common.PORTFOLIO_CSS` is a thin extension.
- `agents/alerts/morning_brief.py` — ONE composed ≤7-line Slack brief to `#daily-alerts` (gate · money line · book · qualified · ER · regime risk + cockpit link), posted by `premarket_alert.py` `__main__` after the gap-up job, even on RED/BLACKOUT. Composes only from existing state files; every line non-fatal. Tests: `tests/test_morning_brief.py`.
- `utils/generators/generate_dashboard.py` — RETIRED (cx-rehaul §A.4): no workflow invokes it; its unique recent-events feed folded into `live_portfolio.html`; last `dashboard.html` snapshot stays linked from the index Report Archive.
...
- `utils/live_check.py` — **live ticker check. Run this before quoting ANY current number.** Every file under `data/` is a snapshot from the last workflow run; `etf_rotation.json` and `finviz_screeners_*.csv` are yesterday's close. On 2026-08-19 XBI was reported to the user as "RS 51, rank 22, mid-pack" straight from the cached snapshot while it was actually breaking to RS 86 / rank 6 — a stale read presented as current fact, which cost a real trade. This module fetches Finviz live at call time and computes ATR mult vs the tier warn/signal; cached values appear only under a `CONTEXT (cached — NOT live)` heading, each stamped with its age in days and flagged `<-- STALE`. `python -m utils.live_check XBI ARKG MRNA`. Tests: `tests/test_live_check.py` (12).
...
- `utils/generators/generate_daily_cockpit.py` — **Daily Cockpit** (`data/daily.html`). Decision-first single pane that replaces the chart "firehose" — designed around the user's documented leaks (round-tripping winners, hold-in-hope on losers, over-trading weak tapes). 6 blocks top→bottom = the morning routine: **0 Discipline banner** (sizing mode · streak · equity vs $150k/$200k goal) · **1 🚦 The Gate** (`gate_decision()` maps market_state + ETF regime + sizing_mode → FULL/HALF/**NO NEW ENTRIES**/PAPER ONLY + position cap; regime `blow-off-risk`/`late-rotation` and `reduced`/`suspended` only ever tighten) · **2 📓 The Book** (live SnapTrade via `fetch_positions` + `verdict_for`, scale-due/cut-due flags made loud; falls back to `positions.json`) · **3 🎯 Qualified Today** (`qualify_setups()` = Ready-to-Enter gate over latest screener CSV, ≤5 cards **ranked by RS Rating, not Quality Score** (Q≥80 is the gate; re-ranking by Q double-counts quality and — since Q is 30pts market cap + 20pts EPS — floats big slow names over leaders: on 2026-08-19 AGNC (Q114, RS 17, mortgage REIT) ranked #1 and NVDA (Q104, RS 25, −0.2% qtr) #8 while SNOW (RS 93, +89% qtr) sat #12 and NESR (RS 93, +375% yr) #17); greyed watch-only when gate closed; "0 qualify = patience") · **4 🎯 Radar** (2026-07-12 revamp, spec [docs/specs/cockpit-radar-revamp.md](docs/specs/cockpit-radar-revamp.md) — Top-5 Entry-Ready + Top-5 Focus tables via `radar_pick`, watching tier removed from cockpit, archived filtered, ranked by (|SMA20%| proximity asc, Q desc), per-row `radar_trigger` line: at-21EMA / pivot ~$X / pullback ~$X / wait-for-reclaim; greyed when gate closed) · **5 🗺 Leadership** (2026-07-12 revamp — flow before structure via `leadership_flows`: 💸 IN = top 5 climbing `rank_delta_5d` with RS ≥ 50 · 💨 OUT = worst 3 · 🎯 Bases worth screening = BASE ∩ (RS ≥ 50 OR improving), weak bases dropped · regime advice + wide-spread suffix when max−min RS ≥ 60) · **6 📊 The Record** (win-rate + avg-win/avg-loss payoff). Watchlist side of the same spec: `_update_watchlist` step 3h enforces `ENTRY_READY_CAP = 5` — active entry-ready rows beyond 5 demote to focus (`demote_reason="entry-ready cap — outranked"`), ranked by the same (proximity, Q). Pure decision/render fns import-safe (unit-tested in `tests/test_daily_cockpit.py`, 19 tests). Regenerated from `position_monitor.py` (live book) AND `daily-finviz.yml` (fresh screener). Linked from `index.html` as first hero button **☀️ Daily Cockpit**. Spec: [docs/specs/daily-cockpit.md](docs/specs/daily-cockpit.md). Slack overhaul (daily Slack is the same firehose) deferred to its own spec.
...
**Position history cache (`data/position_history.json`):** every run pulls 90d of BUY+SELL activities grouped by ticker and **MERGES them into the cumulative cache via `merge_position_history()` — never overwrites** (2026-07-18: the old 90d overwrite orphaned every SELL whose BUY aged out of the window; the live-portfolio FIFO matched AAOI's May sells against a late-May re-buy and rendered the real +$6.2k winner as a −$1.2k May loss, flipping the whole May month negative). Dedup key per ticker: (full-timestamp date, action, shares); fresh record wins on collision so broker price corrections propagate. Cache seeded 2026-07-18 with a full 550d pull (fills back to Jan 2025). Tests: `tests/test_position_monitor.py::TestMergePositionHistory`. Dashboard renders expandable per-row transaction timeline (chevron click) showing avg-up / partial sell / full close with running cost basis.
...
data/
  ...
  paper_stops.json                        # Paper trade state: {ticker: {stop_price, entry_price, atr_pct, entry_date, highest_price_seen, peak_gain_pct, breakeven_activated, target1, target2, target1_hit, pending_close, trail_mode, trail_mode_since, ema21_close_breaches}}
  trail_mode_ab.json                      # 21 EMA trail mode A/B log — per post-close run per ema21-mode position: {date, ticker, close, ema21, atr_stop, atr_would_exit, ema21_exited, breach_count}
...
**📏 Signal Scorecard (2026-07-16 — spec [docs/specs/signal-scorecard.md](docs/specs/signal-scorecard.md)):** measures every screener callout block on forward returns so blocks can be pruned, not just added. Daily side (`finviz_agent.py` §2.1): after all callout blocks compute, `build_signal_fire_records` + `record_signal_fires` log one record per (date, block, ticker) — price/Q/ATR%/rank — to `data/signal_fires.json` (13 blocks in `SIGNAL_BLOCKS`, rolling 400 days, idempotent per day, non-fatal). Weekly side (`finviz_weekly_agent.py` §2.2–2.3): `run_signal_scorecard` fetches Alpaca bars for unscored fires past the 20-session horizon (explicit `start`), scores `ret_5d`/`ret_20d`/`hit10_20d`/`max_drawdown_20d`/excess-vs-SPY, caches outcomes back onto the fire records (scored once), aggregates per block (lifetime excludes `backfilled: true`; trailing 90d) into `data/signal_scorecard.json`, and renders the 📏 weekly HTML table (sorted by median excess, red REVIEW rows) + ≤3-line Slack (best/worst/newly-flagged). Flag rule — informational only, nothing auto-disabled: `n ≥ 20 over ≥8 weeks AND median_excess_20d ≤ 0 AND hit10_rate < 15%` → `REVIEW/RETIRE`. One-off backfill: `scripts/backfill_signal_fires.py` (dry-run default, `--apply`; reconstructs from current state files only, tagged `backfilled: true`). Tests: `tests/test_signal_scorecard.py` (22).
...
carried from the Finviz screener table into `summary_df['Price']` — identified by
*format* via `classify_screener_tail()` (Change ends `%`, Price has a decimal
point, Volume is a comma-grouped integer). **2026-07-12:** Finviz reordered the
table to Price@8/Change@9/Volume@10; the old fixed `cols[9]` read wrote Change%
into Price and the price into Volume, silently fail-opening this gate and zeroing
the Big Movers 9M-share gate — never read these cells by position
(`tests/test_dollar_volume_gate.py::TestClassifyScreenerTail`). Gate runs in `main()` after snapshot
...
**⚡ Momentum Leader (2026-08-19 — TWST-class)** — RS-driven lane for names too strong to look "quiet". Every other Stage 2 block (Ready-to-Enter, Fresh Breakout, Rotation Catalyst, 21 EMA PB) gates on ATR ≤7-8 · RVol ≤1.2 · VCP ≥70 · dist -1..-12 — a profile that describes a *resting* stock. A name up 300%+ on the year is almost never resting, so the system was structurally blind to its own biggest winners. **TWST is the reference case: 15 screener appearances 2026-06-24 → 2026-08-13 at RS 87-98, never once admitted to a Slack block or the watchlist, while it ran $99 → $125+ (+337% Y/Y).** Predicate (`_is_momentum_leader`): Stage 2 (perfect NOT required — leaders dip the SMA20 on the way up) · **RS Rating ≥ 85** (`MOMENTUM_LEADER_RS_MIN`) · Q ≥ 60 (`MOMENTUM_LEADER_Q_MIN` — pre-profit biotech and recent IPOs cap out well below the ≥80 quiet-setup bar on EPS/market-cap points) · dist from 52w high ≥ -15% (leading, not repairing) · **NO ATR cap, NO RVol cap — that is the entire point** · **peel-safe kept HARD** (user decision 2026-08-19: it is the one gate stopping this from becoming a top-chasing block — it correctly held TWST out on 06-26 at mult 8.63 and admitted it on 08-04 at mult 2.20) · not held · not in an earlier callout. Ranked by **RS desc** (leadership, not setup tightness), Q breaks ties, top 5 in Slack block "⚡ Momentum Leader" framed watch-only/size-half (wide ATR ⇒ wider stop). Gallery: `<details open>` section with purple `RS {n}` badge. Watchlist: auto-enters at its **own distinct tier `priority=momentum`** (`source=momentum_leader_auto` — **10th entry path**), so a high-ATR leader is never confused with a quiet coiled setup; renders as its own ⚡ card on watchlist.html and its own third column on the cockpit Radar. 48-day replay (2026-06-10 → 08-18): TWST first surfaces **2026-08-03 @ $101.06** (now ~$125+). Tests: `tests/test_momentum_leader.py` (14).
