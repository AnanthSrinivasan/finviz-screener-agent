...
**Recent events feed (`data/recent_events.json`):** rolling last 50 dashboard-surfaced events. Schema: `{updated, events: [{ts, date, category, title, severity, detail?}]}`. **Market events only** — categories: `market_state` (market_monitor) and other regime/breadth events. Position events (stop_hit, breakeven, target_hit, position_close) deliberately do NOT write here — they go to Slack only. The Apr 29 2026 port removed all position-event writes from `apply_minervini_rules` and the auto-close branch per spec. Helper `_append_recent_event` lives in `utils/events.py` (shared, DATA_DIR-aware); called only from `market_monitor.py` on state change. The dashboard "Recent Alerts" widget reads this file (newest 10) and falls back to legacy `alerts_state.last_alerts_sent` only if empty. Severity values: `low` (green), `med` (amber), `high` (red) → CSS left-border color. **Durability (2026-08-19):** the file is written atomically (tmp + `os.replace`) and a corrupt read is salvaged via `json.JSONDecoder().raw_decode` and rewritten rather than dropped. Before this, one stray trailing `}` made `json.load` raise into the catch-all handler, so the append was discarded with only a warning and the feed could never heal — it sat frozen from 2026-04-27 to 2026-08-19, silently losing every market-state transition in between (including 2026-08-18 TREND-FOLLOW → RED) from the Live Portfolio panel at `generate_live_portfolio.py:355`. Never let a read failure on this file fall through to the outer `except`. Tests: `tests/test_events.py`.
...
**Position history cache (`data/position_history.json`):** every position-monitor run, `fetch_position_history(account_ids, days=90)` pulls all BUY+SELL activities, groups by ticker, and **merges into the cumulative cache via `merge_position_history(existing, fresh)` — never overwrites** (2026-07-18: the old 90d overwrite aged BUY legs out of the window, so the live-portfolio FIFO orphaned later SELLs — AAOI's real +$6.2k realized rendered as a −$1.2k May loss and May 2026 flipped from +$4.8k to −$2.7k on the page. Dedup key per ticker: full-timestamp date + action + shares; fresh wins on collision so broker price corrections propagate. Seeded with a 550d pull, fills back to Jan 2025. Tests: `tests/test_position_monitor.py::TestMergePositionHistory`). File format: `{updated, history: {ticker: [{date, action, shares, price}]}}`. Paginated via `offset`/`limit=200` with cross-page dedup by activity id. **File must be in the `git add` list of `position-book.yml` and `position-critical.yml`** — was missing originally (May 2026), so the CI-written file was never pushed back. Locally it stayed frozen at the last manual commit while live SnapTrade SELL fills (AAOI/GLW) were being correctly fetched but discarded at workflow end. Dashboard $P/L walk had nothing to walk against. Fixed: both workflows now include `data/position_history.json` in commit. Used by the dashboard generator to render an expandable transaction timeline (chevron toggle) per open and closed position — shows avg-up, partial trim, full close events with running cost basis.
...
- **⚡ Momentum Leader** (`_is_momentum_leader`, 2026-08-19 — TWST class): RS-driven lane for names too strong to look "quiet". Every other Stage 2 callout gates on ATR ≤7-8 · RVol ≤1.2 · VCP ≥70 · dist -1..-12, a profile describing a **resting** stock; a name up 300%+ on the year is almost never resting, so the system was structurally blind to its own biggest winners. **Reference case:** TWST appeared in the screener 15× between 2026-06-24 and 2026-08-13 at RS 87-98 and never once reached a Slack block, the gallery, or the watchlist while it ran $99 → $125+ (+337% Y/Y). Criteria: Stage 2 (perfect NOT required — leaders dip the SMA20 on the way up) · `RS Rating ≥ MOMENTUM_LEADER_RS_MIN` (**85**) · `Q ≥ MOMENTUM_LEADER_Q_MIN` (**60** — pre-profit biotech and recent IPOs cap out well below the ≥80 quiet-setup bar on EPS/market-cap points) · dist from 52w high ≥ **-15%** (leading, not repairing) · **no ATR cap, no RVol cap — that is the entire point of the lane** · **peel-safe kept HARD** · not held · not in an earlier callout. ⚠ **Do not relax peel-safe here.** It is the only gate preventing this from becoming a top-chasing block: it correctly excluded TWST on 2026-06-26 at mult 8.63 and admitted it on 2026-08-04 at mult 2.20 (user decision 2026-08-19, alongside RS 85 over 90). Ranked by **RS desc** (leadership, not setup tightness; Q breaks ties), top 5 in Slack block `⚡ Momentum Leader` with watch-only / size-half framing — a wide ATR implies a wider stop than a VCP setup. Gallery: `<details open>` section with purple `RS {n}` badge. Watchlist: **10th entry path**, auto-enters at its own distinct tier `priority=momentum` (`source=momentum_leader_auto`) rather than `watching`/`focus`, so a high-ATR leader is never confused with a quiet coiled setup — renders as its own ⚡ card in the watchlist category grid and its own third column on the cockpit Radar. 48-day replay (2026-06-10 → 08-18): TWST first surfaces 2026-08-03 @ $101.06. Tests: [tests/test_momentum_leader.py](tests/test_momentum_leader.py) (14).
...
**Age-out pass (3b + 3g-2, broadened 2026-08-14):** two decays keep the list from becoming a graveyard.
- **3b — watching age-out:** any `priority=watching` entry from *any auto source* older than 14 days (by `added`) is archived (`archive_reason=age_out`). **Was restricted to `source=='screener_auto'`** — that guard let every other auto path (hidden_growth/breakout/rs_leader/rotation/stage_transition/ema21_pb/htf_base_reclaim) pin names to `watching` forever, the core of the 2026-08 bloat (420 active rows, ~280 mislabeled "manual" — see the badge fix below). Only a literal `source='manual'` is exempt.
- **3g-2 — focus cold-archive:** a `priority=focus` entry absent from the screener ≥`FOCUS_STALE_TDAYS=15` trading days (clock = `last_seen_in_screener` fallback `added`) is archived (`archive_reason=age_out` so it can reactivate). Focus previously *never* decayed ("earned their place") but a name that hasn't screened in ~3 weeks is no longer actionable-this-week. Skips names in today's screener (`last_seen` refreshed in 3f) and any row `demoted_from_entry_ready_date==today` (a name soft-landed entry-ready→focus this run keeps its full focus grace — don't kill it same-run on a pre-demotion `last_seen`).
- **One-time prune 2026-08-14:** applied the same two rules to `data/watchlist.json` — 194 stale rows archived (56 watching + 138 cold focus), active 422 → 228. Commit `fa19b18`.
- **Badge label fix (`generate_watchlist._row`):** the watchlist.html source badge rendered "auto" only for `screener_auto`/`weekly_auto`, so ~280 system-added rows showed as "manual". Now any non-`manual` source → "auto"; only a literal `source='manual'` → "manual". Tests: `tests/test_watchlist_lifecycle.py` (4 new cases: focus cold-archive fires, freshly-demoted skip, non-screener_auto watching ages out, manual never ages out).
...
Priority lifecycle: `watching → focus → entry-ready` (and optionally `archived` from any). Auto-archive paths: `watching` ages out at 14 calendar days (3b), `focus` cold-archives at 15 trading days absent from the screener (3g-2), `entry-ready` demotes to `focus` at 5 trading days absent (3g) rather than archiving. `manual` rows never auto-archive.
