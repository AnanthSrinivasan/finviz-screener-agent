...
- **Performance dashboard ([utils/generate_performance.py](utils/generate_performance.py)):** `load_snaptrade_partial_realized(data/position_history.json)` emits one synthetic realized trade per ticker with `total_sold_units > 0`. Walk supersedes the synthesized `load_system_closed` entry for overlapping tickers — `closed_positions` only records the FINAL-tranche shares (e.g. AAOI 10 sh × +86%) while the walk includes every prior partial SELL in the cycle (AAOI 90 sh sold → +$6,949 realized). Tagged `close_source = snaptrade_walk`.
